...
class SessionDB extends Session
{
   ...
   {
      $res = $this->_db->val( 'SELECT data FROM ' . $this->_table . ' WHERE sid = ' . $this->_db->str( $sid ) . ' LIMIT 1' );
      return $res ? $res : '';
   }
   ...
   public function write( $sid, $data )
   {
      ...
      $this->_db->query( 'INSERT INTO ' . $this->_table . ' (sid,data,mtime,uid) VALUES (' . $this->_db->str( $sid ) . ', ' . $this->_db->str( $data ) . ',' . $timestamp . ',' . $this->uid . ')' .
   ...
   {
      $this->_db->query( 'DELETE FROM ' . $this->_table . ' WHERE sid = ' . $this->_db->str( $sid ) . ' LIMIT 1' );
      return TRUE;
   }
