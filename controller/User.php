...
// user logout
   ...
   public function deleteAction()
   {
      ...
      $this->cache->setStatus(FALSE);
      $uid = $this->request->args[1];
      if ($this->request->uid == 1 && $uid > 2)  // can not delete uid = 1
      {
         $user = new UserObject();
         $user->uid = (int) $uid;
         $user->delete();
...
//logged in user
   public function editAction()
   {
      ...
      if (empty($this->request->post))
      {
         ...
         $avatar = new Input('avatar', '用户头像', '您的虚拟头像。最大尺寸是 <em>90x90</em> 像素，最大大小为 <em>30</em> KB');
         $avatar->type = 'file';
      ...
      else
      {
         ...
         if ($file['error'] == 0 && $file['size'] > 0)
         {
            ...
            if ($fileInfo === FALSE || $fileInfo[0] > 90 || $fileInfo[1] > 90)
            {
               $this->error('修改头像错误：上传头像图片尺寸太大。 ');
               return;
            }
            else
            {
               $avatar = '/data/avatars/picture-' . $uid . image_type_to_extension($fileInfo[2]);
