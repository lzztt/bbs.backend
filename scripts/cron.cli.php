...
function do_user($path, $logger, $config)
{
   ...
   if (\sizeof($users) > 0)
   {
      ...
      foreach ($users as $u)
      {
         ...
         $logger->info('activate user: ' . $u['uid'] . ' : ' . $password);
      ...
      if ($mailer->send() === FALSE)
      {
         $logger->info('sending new user activation email error.');
      }
   }
}
...
function do_alexa($path, $logger, $config)
{
   ...
   if ($contents)
   {
      ...
      if ($p[2])
      {
         ...
         \file_put_contents($path['theme'] . '/' . $config->theme . '/pc/alexa.tpl.php', $data);
      }
      else
      {
