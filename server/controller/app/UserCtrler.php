...
class UserCtrler extends App
{
   ...
   public function getLatestVersion()
   {
      ...
      try
      {
         $current = \file_get_contents( $this->config->path[ 'file' ] . '/app/user.current' );
      }
      catch ( \Exception $ex )
      {
         // ignore and continue
      ...
      // need to search
      $dirs = \glob( $this->config->path[ 'file' ] . '/app/user.*', \GLOB_ONLYDIR );
      // not found
      if ( !$dirs )
      {
         $this->pageNotFound();
      ...
      try
      {
         \file_put_contents( $this->config->path[ 'file' ] . '/app/user.current', \basename( $dir ) );
      }
      catch ( \Exception $ex )
      {
         // ignore and continue
