...
class StartCtrler extends Lottery
{
    public function run()
    {
        ...
        if (empty($user->firstname) || $bmonth * $bday == 0) {
            $this->var['content'] = '<div class="messagebox">个人认证信息不全，您需要提供您的名字(First Name)和生日(Month & Day)后才能抽奖<br />'
        ...
        if ($this->request->timestamp < $t_start) {
            ...
            $minutes = floor($t / 60);
            $seconds = $t % (60);
            $this->var['content'] = '<div class="messagebox">正式抽奖将从<span class="highlight">2011年12月16日 00点00分00秒</span>(美国中部时间)开始<br />'
            ...
            $round = 0;
            $isActive = false;
            $this->var['content'] = '<div class="messagebox">抽奖已于2012/01/18 21点结束，您现在不能进行抽奖。</div>';
        }
        ...
        if ($this->args[0] === 'run') {
            if ($isActive !== true) {
                $this->var['content'] = '<div class="messagebox">您现在不能进行抽奖。</div>';
            }
            ...
            if ($this->request->timestamp - $lastLotteryTime < 60) {
                $this->var['content'] = '<div class="messagebox">您的动作太快了，两次抽奖的间隔时间最少为60秒钟<br />请稍候点击下面抽奖按钮再试<br /><a class="bigbutton" href="/lottery/start/run">点击抽奖</a></div>';
            }
        ...
        $this->var['content'] = new Template('lotteryStart', ['average' => $average, 'aPoints' => $aPoints, 'results' => $results]);
    }
}
