...
class Single extends Controller
{
   ...
   public function showAction()
   {
      ...
      {
         $db->insert( 'INSERT INTO ff_counts (sid, count) VALUES (' . $db->str( session_id() ) . ', 1)' );
      }
      else
      {
         $db->update( 'UPDATE ff_counts SET count = count + 1 WHERE sid = ' . $db->str( session_id() ) );
      }
   }
...
//       echo '报名成功，但因为8月6号的七夕活动已经结束，确认邮件并未发送。您将在下次活动计划出来时收到带有活动详情的电子邮件确认';
   ...
   {
      $db = $this->db;
      $comments = $db->select( 'SELECT name, body, time FROM ff_comments WHERE time > ' . $this->thirty_two_start . ' ORDER BY id ASC' );
      $comments_thirty = $db->select( 'SELECT name, body, time FROM ff_comments WHERE time > ' . $this->thirty_start . ' AND time < ' . $this->thirty_two_start . ' ORDER BY id ASC' );
      $comments_rich = $db->select( 'SELECT name, body, time FROM ff_comments WHERE time > ' . $this->rich_start . ' AND time < ' . $this->thirty_start . ' ORDER BY id ASC' );
   ...
   {
      $sql = 'SELECT'
            . ' (SELECT count(*) FROM ff_counts) AS visitorCount,'
            . ' (SELECT sum(count) FROM ff_counts) AS hitCount,'
            . ' (SELECT count(*) FROM ff_comments) AS commentCount,'
            . ' (SELECT count(*) FROM ff_attendees WHERE time > 1376629987) as attendeeCount,'
            . ' (SELECT count(*) FROM ff_attendees WHERE sex = 1 and time > 1376629987) as maleCount,'
            . ' (SELECT count(*) FROM ff_attendees WHERE sex = 0 and time > 1376629987) as femaleCount,'
            . ' (SELECT count(*) FROM ff_subscribers) as subscriberCount';
      $r = $this->db->row( $sql );
