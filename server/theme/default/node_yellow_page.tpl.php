  ...
  <div id="content-area">
    <div class="node-type-yp" id="node-2622">
      <div class="node-inner">
        ...
        <div id="content-header"><?= $breadcrumb ?></div>

        <div class="forum-top-links">
          <ul class="links forum-links">
            <li data-urole='<?= $urole_user ?>'><a rel="nofollow" class="reply button" href="/node/<?= $nid ?>/comment">评论</a></li>
            <li data-urole='<?= $urole_guest ?>'>您需要先<a rel="nofollow" href="/user">登录</a>或<a href="/user/register">注册</a>才能发表评论</li>
          </ul>
        </div>
      ...
      <div class="bcard">
        <table>
          <tbody>
            <tr><td>地址:</td><td><?= $node['address'] ?></td></tr>
            <tr><td>电话:</td><td><?= $node['phone'] ?></td></tr>
            <?php if (isset($node['fax'])): ?>
              <tr><td>传真:</td><td><?= $node['fax'] ?></td></tr>
            <?php endif ?>
            <?php if (isset($node['email'])): ?>
              <tr><td>电邮:</td><td><?= $node['email'] ?></td></tr>
            <?php endif ?>
            <?php if (isset($node['website'])): ?>
              <tr><td>网站:</td><td><a href="<?= $node['website'] ?>" target="_blank"><?= $node['website'] ?></a></td></tr>
            <?php endif ?>
          </tbody>
        </table>
      </div>
      <div style="padding: 8px 0;"><?= $node['HTMLbody'] ?></div>
      ...
      <div>
        <?php for ($i = 1; $i <= 10; $i++): ?>
          <input name="star-avg" type="radio" class="star {split:2}" disabled="disabled" <?= (((int) round($node['ratingAvg'] * 2)) == $i) ? 'checked="checked"' : '' ?>/>
        <?php endfor ?>
        <span> (<?= '平均分：' . round($node['ratingAvg'], 1) . '，' . $node['ratingCount'] . '人评过分' ?>)</span>
      </div>
      ...
      <div class="comments-node-type-yp" id="comments">
        ...
        <?php foreach ($comments as $c): ?>
          ...
          <div class="comment comment-published odd first">
            ...
            <div class="submitted">
              由 <a title="浏览用户信息" href="/user/<?= $c['uid'] ?>"><?= $c['username'] ?></a> 发表于 <?= ($c['createTime']) ?><?= ($c['lastModifiedTime']) ? '，最后修改于' . ($c['lastModifiedTime']) : '' ?>。  </div>

            <div>
              <?php for ($i = 1; $i <= 5; $i++): ?>
                <input name ="star_<?= $c['id'] ?>" type="radio" class="star" disabled="disabled" <?= ($c['rating'] == $i) ? 'checked="checked"' : '' ?>/>
              <?php endfor ?>
            </div>

            <div class="content" style="clear:both;"><?= $c['HTMLbody'] ?></div>

            <div class="post-links user">
              <ul class="links inline forum-links">
                <li><a rel="nofollow" title="Edit" class="edit button" data-urole="<?= $urole_adm . $tid . ' ' . $urole_user . $c['uid'] ?>" id="<?= $c['type'] . '-' . $c['id'] ?>-edit" href="<?= '/' . $c['type'] . '/' . $c['id'] . '/edit' ?>">编辑</a></li>
                <li><a rel="nofollow" title="Delete" class="delete button" data-urole="<?= $urole_adm . $tid . ' ' . $urole_user . $c['uid'] ?>" id="<?= $c['type'] . '-' . $c['id'] ?>-delete" href="<?= '/' . $c['type'] . '/' . $c['id'] . '/delete' ?>">删除</a></li>
              </ul>
            </div>
            <div id="<?= $c['type'] . '-' . $c['id'] ?>-raw" style="display:none;">
              <pre><?= $c['body'] ?></pre>
            </div>
          </div>
        <?php endforeach ?>
    ...
    <div class="box">
      <div class="box-inner" data-urole='<?= $urole_user ?>'>
        <h2 class="title">发表评论</h2>
        <div id="editor-div">
          <form id="editor-form" method="post" accept-charset="UTF-8" action="/node/comment/<?= $nid ?>">
            <div>
      ...
      <div class="box-inner" data-urole='<?= $urole_guest ?>'>
