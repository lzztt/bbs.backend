    ...
    <tbody>
      <?php foreach ($comments as $i => $c): ?>
        ...
        <tr  class="<?= ($n % 2 == 0) ? 'even' : 'odd' ?>">
          <td class="commentTableAuthor"><?= '<b>' . $c['name'] . '</b><br />' . (date('m/d/Y H:i', $c['time'])) ?></td>
          <td class="commentTableFloor"><?= $n ?></td>
          <td><?= nl2br($c['body']) ?></td>
        </tr>
      <?php endforeach ?>
      ...
      <?php foreach ($comments_thirty as $i => $c): ?>
        <?php $n = $i + 1 ?>
        <tr  class="<?= ($n % 2 == 0) ? 'even' : 'odd' ?>">
          <td style="background-color: #CCCCCC;" class="commentTableAuthor"><?= '<b>' . $c['name'] . '</b><br />' . (date('m/d/Y H:i', $c['time'])) ?></td>
          <td class="commentTableFloor"><?= $n ?></td>
          <td style="background-color: #CCCCCC;"><?= nl2br($c['body']) ?></td>
        </tr>
      <?php endforeach ?>
